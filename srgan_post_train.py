...
def main():
    ...
    dataset_lr = TensorflowImageDataset(data_dir_low_res, normalizer=lambda x: normalize_input(x))
    dataset_hr = TensorflowImageDataset(data_dir_high_res, normalizer=lambda x: normalize_input(x))
    ...
    with tf.device('/GPU:2'):
        ...
        generator.load_weights('./.cache/models/srgan-new/generator.weights.h5')
        discriminator.load_weights('./.cache/models/srgan-new/discriminator.weights.h5')
        ...
        trainer.train(dataset, epochs=num_epochs, steps=batched_data_set_cardinality)
